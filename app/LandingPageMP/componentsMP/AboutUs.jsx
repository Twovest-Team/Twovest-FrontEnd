            ...
            <h6 className="text_h6 text-center">{title}</h6>
            {
                typeof(text) == "string" ? (
                    ...
                    <ul className='list-disc'>
                        {text.map((item, index) => (
                            <li key={index} className="text_caption text-secondary text-sm">{item}</li>
                        ))}
                    </ul>
                )
            }
...
export default function AboutUs () {
    return(
        <>
        <div className="w-[388px] h-[57px] py-2.5 justify-center pt-[50px] items-center inline-flex">
      ...
      <div className='px-[73.35px]'>
        <Image src={PhoneImage} alt="PhoneImage" width={211.86} height={268.43} className='rotate-[-6.6deg]'/>
      </div>
        </>
    )
}