...
const Form = ({ params }) => {
  ...
  const [selectedOffersIds, setSelectedOffersIds] = useState([]);
  ...
  const handleSubmitLook = async () => {
    ...
    try {
      await teste(userId, selectedProductIds, selectedOffersIds, selectedStyleIds, gender.string, gender.id, formData, lookImageAlt, lookImageURL);
    } catch (error) {
    ...
    <div className="container mx-auto overflow-hidden mb-24 md:w-[650px]">
      ...
      <div className="mb-4" id="Instagram">
        ...
        {!isInstagramURLValid ? (
          ...
          lookImageURL.length > 5 && <p className="text-primary_main text-sm mt-1 ml-1">URL válido.</p>
        )}
      </div>
