...
const selectedIcon = iconMap[icon] || null;
  ...
  return (
    <button className={`flex font-semibold font-inter aria-label="${text}" rounded ${sizes[btnSize]} ${StyledState[btnState]} mx-auto items-center`}>
    
       {['google', 'facebook', 'apple', 'redifine'].includes(icon) && <span>{selectedIcon}</span>}
       <span className={`text-white  ${!selectedIcon ? 'text-center' : ''}`}>{text}</span>
