...
export const CardCart = ({ handleShowDeleteNotification, data, userEmail, handleLoading }) => {

    const dispatch = useDispatch()
    const categoryName = categories.find(element => element.id === data.offers.products.categories.id).singular
        ...
        <article className='py-12 border-b border-grey'>
            <div className="flex self-center items-center w-full">
                ...
                <figure className="bg-white border min-w-[115px] aspect-square border-grey rounded relative">
                    <Image src={data.offers.images[0].url} width={115} height={115} alt={data.offers.images[0].alt} />
                    ...
                    <div className="ml-4 flex flex-col font-semibold justify-between min-w-0 flex-grow ">
                        <div className='flex flex-col gap-1'>
                            ...
                            <div className='flex justify-between gap-2'>
                                <p className='truncate'>{categoryName} {data.offers.products.brands.name}</p>
                            ...
                            <p className={`caption
                                ${data.offers.conditions.id === 1 && 'text-primary_main'}
                                ${data.offers.conditions.id === 2 && 'text-info_main'}
                                ${data.offers.conditions.id === 3 && 'text-warning_main'}
                            `}>

                                {data.offers.conditions.name
                                }</p>
                            <p className="text-secondary font-normal caption">Tamanho: {data.offers.sizes.size}</p>
                        </div>
                        ...
                        <div className='flex justify-between'>
                            <p className="font-semibold h-8 flex items-center">
                                {data.offers.products.discount > 0 ?
                                    <>
                                        {applyPriceDiscount(data.offers.price, data.offers.products.discount)}
                                    </> :
                                    <>
                            ...
                            <div className='hidden min-[350px]:block'>
                                <ProductQuantityControl cartId={data.id} userEmail={userEmail} qty={data.qty} handleLoading={handleLoading} />
                            </div>
                        </div>
