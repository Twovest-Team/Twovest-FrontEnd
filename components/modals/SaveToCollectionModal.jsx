...
const CollectionChoosing = ({ collections, getData, allModalStatus, setModalStatus, lookId }) => {
    ...
    return (
        <>
            ...
            <button onClick={() => setModalStatus(allModalStatus[1])} className='bg-dark w-full py-4 font-semibold px-6 flex justify-between items-center text-white rounded'>
                Criar nova coleção
                <AddIcon />
            </button>
...
const CollectionButton = ({ collection, lookId, getData }) => {
    ...
    return (
        <button
            ...
            className={` ${saveStatus === 'loading' && 'opacity-20 pointer-events-none'}  transition-opacity duration-75 w-full min-w-0 flex items-center justify-between border-2 rounded border-black px-6 h-[60px] font-semibold`}>


            <div className='text-center w-full flex justify-between'>
                <div className='flex items-center gap-2 truncate'>
                    
                    {collection.collections.privacy === 1 && <HttpsOutlinedIcon sx={{ fontSize: 20 }} />}
                    {collection.collections.privacy === 2 && <PublicOutlinedIcon sx={{ fontSize: 20 }} />}
                    {collection.collections.privacy === 3 && <GroupAddOutlinedIcon sx={{ fontSize: 20 }} />}
...
const ToCreate = ({ userHasAnyCollection, allModalStatus, setModalStatus, lookId, userId, handleCloseModal }) => {
    ...
    return (
        <form onSubmit={e => handleCreateCollection(e)} className='flex flex-col gap-6'>

            <div className='container shadow-lg rounded py-6 border border-grey flex flex-col gap-4'>
                <div>
                    <h6 className="font-semibold">{name.trim().length === 0 ? 'Nova coleção' : name}</h6>
                ...
                <input required onChange={(e) => setName(e.target.value)} value={name} type="text" name="name" id="name" placeholder='Nome da coleção' className='border border-grey rounded px-6 py-4 w-full' />
            </div>

            <RadioGroup className='flex flex-col gap-4' value={privacy} onChange={setPrivacy}>
                <RadioGroup.Option value={1}>
                    {({ checked }) => (
                        <div className={`cursor-pointer font-semibold flex items-center gap-5 border-2 px-5 py-4 rounded ${checked ? 'bg-grey_opacity_50 border-black' : 'border-grey'}`}>
                            <HttpsOutlinedIcon sx={{ fontSize: 28 }} />
                            <div className='flex flex-col justify-center'>
                                <p>Privada</p>
                                <p className='text-secondary font-normal caption'>Esta coleção ficaria visível só para ti.</p>
                            </div>
                        </div>
                    )}
                </RadioGroup.Option>
                <RadioGroup.Option value={2}>
                    {({ checked }) => (
                        <div className={`cursor-pointer font-semibold flex items-center gap-5 border-2 px-5 py-4 rounded ${checked ? 'bg-grey_opacity_50 border-black' : 'border-grey'}`}>
                            ...
                            <div className='flex flex-col justify-center'>
                                <p>Pública</p>
                                <p className='text-secondary font-normal caption'>Qualquer pessoa que visite o teu perfil.</p>
                            </div>
                        </div>
                    )}
                </RadioGroup.Option>
                <RadioGroup.Option value={3} disabled={true}>
                    {({ checked }) => (
                        <>
                            <div className={`cursor-pointer opacity-40 font-semibold flex items-center gap-5 border-2 px-5 py-4 rounded-t ${checked ? 'bg-grey_o border-blackpacitborder-greyy_50' : ''}`}>
                                <GroupAddOutlinedIcon sx={{ fontSize: 28 }} />
                                <div className='flex flex-col justify-center'>
                                    <p>Partilhada</p>
                                    <p className='text-secondary font-normal caption'>Partilhar esta coleção com até +4 pessoas.</p>
                                </div>
            ...
            <button className="bg-dark block text-center text-white h-[60px] font-semibold rounded">
                {!loading ? 'Criar coleção e guardar look' :
            ...
            <button onClick={() => setModalStatus(allModalStatus[0])} className='text-error_main underline underline-offset-2 font-semibold'>Cancelar</button>
